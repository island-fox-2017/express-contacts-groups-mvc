class Contacts {
  ...
  static forDetailGroup(conn, param, callback){
    conn.all(`SELECT * FROM contacts c LEFT JOIN contactgroup cg ON c.id = cg.contact_id LEFT JOIN groups  g ON g.id = cg.group_id where c.id = ${param}`, function(err, rows){
      if (!err) {
        callback(false, rows)
      } else {
        callback(true, null)
