...
app.get('/my-contact-groups/delete/:id', function (req, res) {
  db.run(`delete from contact_group where group_id=${req.params.id}`);
  res.redirect('/my-contact-groups')
})

app.get('/my-contact-groups/edit/:id', function (req, res) {
  db.all(`SELECT * from CONTACT2017`, function (err, datakontak) {
    db.all(`select * from GROUPS`, function (err, datagroup) {
      res.render('my_group_edit', {datakontak: datakontak, datagroup: datagroup})
    })
  })
})
